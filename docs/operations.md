...
3. Hermes runtime이 `NEWS_SOURCE_URLS`를 수집하고 `{ articles }`로 정규화
4. Hermes webhook 요약
...
뉴스 수집은 `GET /news/articles`에서 확인할 수 있다. `NEWS_SOURCE_URLS`는 쉼표로 구분된 JSON source URL 목록이며, 각 source는 배열 또는 `{ "articles": [...] }`, `{ "items": [...] }`, `{ "data": [...] }` 형태를 반환해야 한다.
