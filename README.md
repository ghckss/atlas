...
Gateway 연결에는 `DISCORD_BOT_TOKEN`, `DISCORD_BOT_USER_ID`가 필요합니다. Discord mention 대화는 봇이 접근 가능한 서버 내 모든 채널에서 동작합니다. Discord Developer Portal에서 Message Content Intent도 활성화해야 mention 메시지 내용을 읽을 수 있습니다.
...
기존 PostgreSQL `schedule_events` 데이터가 있다면 Google Calendar로 1회 마이그레이션한다.
...
뉴스는 기본적으로 `NEWS_PROVIDERS=google-news-top`을 사용해 Google News Top Stories를 수집합니다. 관심 키워드 검색을 추가하려면 `NEWS_PROVIDERS=google-news-top,google-news`와 `NEWS_QUERY`를 설정합니다. `NEWS_SOURCE_URLS`는 수동 JSON/RSS source를 추가할 때만 사용합니다.
