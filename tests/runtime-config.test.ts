...
test("runtime config loads required local MVP settings", () => {
  const config = loadRuntimeConfig({
    ...
    NEWS_PROVIDERS: "google-news, source-url",
    NEWS_QUERY: "AI agent",
  ...
  assert.deepEqual(config.news.providers, ["google-news", "source-url"]);
...
test("runtime config rejects unsupported news providers", () => {
  assert.throws(
    () =>
      loadRuntimeConfig({
        DISCORD_BOT_USER_ID: "bot-1",
